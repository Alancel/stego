...
{
filename = (char*)malloc(sizeof(char) * 1024); //выделение памяти
if (filename == NULL) 
{
...
// all samples to array

int data[num_samples];

// read each sample from data chunk if PCM (type == 1)
...
char *str = message_encode("lalala", num_samples, data);
return 0;
}
